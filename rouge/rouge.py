...
class Rouge:
    ...
    def __init__(self, metrics=None, stats=None, stem=False, remove_stop=False):
        self.metrics = metrics if metrics is not None \
            else Rouge.DEFAULT_METRICS
        self.stats = stats if stats is not None \
            else Rouge.DEFAULT_STATS

        self.rouge_calc = RougeCalculator(stopwords=remove_stop, stemming=stem, lang="en")
    ...
    def _get_avg_scores(self, hyps, refs):
        ...
        for (hyp, ref) in zip(hyps, refs):
            ...
            hyp = [" ".join(_.split()).strip() for _ in hyp.split(" . ") if len(_) > 0]
            ref = [" ".join(_.split()).strip() for _ in ref.split(" . ") if len(_) > 0]
